...
/// </returns>
/// 
inline static std::int32_t cmain(lib_fm::main_arguments);
...
namespace lib_fm
{
	//inline namespace {
		class main_forwarder {
			friend int ::main(int count, char* args[]);
			inline int static forward_arguments(int count, char* args[]);
		};// !main_forwarder
	//};// !namespace
};// !lib_fm

int main(int count, char* args[]) {
	return 	lib_fm::main_forwarder::forward_arguments(count,args);
};

int lib_fm::main_forwarder::forward_arguments(int count, char* args[])
{
	return ::cmain({ count,args });
};// !forward_arguments
...
/// other values can define the error/exception number
/// </returns>
inline static std::int32_t cmain(lib_fm::command_line, lib_fm::environment);
